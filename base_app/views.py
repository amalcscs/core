...
def Trainer_Current_Assigned(request, id):
    ...
    mem = user_registration.objects.filter(
        designation_id=usernametm) .filter(fullname=usernametm1)
    d = create_team.objects.get(id=id)
    vars = trainer_task.objects.filter(id=d.id).order_by('-id')
...
def trainer_current_trainees(request):
    ...
    z = user_registration.objects.filter(designation_id=usernametrnr,fullname=usernametrnr1,id=usernametrnr2) 
...
def trainee_dashboard_trainee(request):
    ...
    z = user_registration.objects.filter(designation_id=usernametrns).filter(
        fullname=usernametrns1).filter(id=usernametrns2).filter(team_id=usernametrns3)
    labels = []
    data = []
...
def trainee_currentTopic(request):
    ...
    mem = topic.objects.filter(
        team_id=usernametrns3) .filter(topic_status=0).order_by('-id')
...
def trainee_completed_taskList(request):
    ...
    mem = trainer_task.objects.filter(
        user_id=usernametrns2).filter(task_status='1').order_by('-id')
...
def trainee_report_reported(request):
    ...
    z=user_registration.objects.filter(designation_id=usernametrns).filter(fullname=usernametrns1)
    var = reported_issue()
    if request.method == 'POST':
...
def trainer_applyleave_cards(request):
    ...
    if request.session.has_key('usernametrnr2'):
        ...
        usernametrnr1 = "dummy"
    z = user_registration.objects.filter(designation_id=usernametrnr,fullname=usernametrnr1)
...
def logout(request):
    try:
        del request.session['usernametrns1']
    except:
...
def trainee_applyleave_cards(request):
    ...
    if request.session.has_key('usernametrns3'):
        ...
        usernametrns1 = "dummy"
    z = user_registration.objects.filter(designation_id=usernametrns,fullname=usernametrns1)
...
def trainees_attendance_viewattendancelist(request):
    ...
    if request.session.has_key('usernametrns3'):
        ...
        usernametrns1 = "dummy"
    z = user_registration.objects.filter(
        designation_id=usernametrns,fullname=usernametrns1) 
...
def admin_intern_newreg(request):
    ...
    mem = user_registration.objects.filter(id=m_id)
    return render(request, 'admin_intern_newreg.html',{'mem':mem})
...
def admin_leaveform(request):
    leave = leave.objects.order_by('-id')
...
def man_registration_form(request):
    ...
    branch = branch_registration.objects.all()
    return render(request, 'man_registration_form.html',{'branch':branch,'mem':mem})
...
def man_registration_formsave(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'man_registration_form.html',{'mem':mem})
...
def man_registration(request):
    if request.session.has_key('m_id'):
        ...
        variable="dummy"
    mem = user_registration.objects.filter(id=m_id)
    return render(request,'man_registration.html',{'mem':mem})
...
def man_registration_update(request):
    if request.session.has_key('m_id'):
        ...
        variable="dummy"
    mem = user_registration.objects.filter(id=m_id)
    return render(request,'man_registration_update.html',{'mem':mem})
...
def man_internship(request):
    ...
    var1=internship.objects.all()
    mem = user_registration.objects.filter(id=m_id)
    return render(request,'man_internship.html',{'mem':mem,'var1':var1})
...
def man_internship_date(request):
    ...
    mem = user_registration.objects.filter(id=m_id)
    return render(request,'man_internship_date.html',{'mem':mem})



def man_internship_update(request,id):
    if request.session.has_key('m_id'):
        m_id = request.session['m_id']
 
    mem = user_registration.objects.filter(id=m_id)
    var=internship.objects.get(id=id)
    return render(request,'man_internship_update.html',{'mem':mem,'var':var})
...
def registartion_detailupdate(request,id):
